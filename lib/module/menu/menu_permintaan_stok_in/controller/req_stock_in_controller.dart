...
import '../view/component/preview_print_req_in.dart';

class ReqStockInController extends GetxController {
  var pageIdx = 0.obs;
  ...
  var lsQtyStockIn = List<TextEditingController>.empty(growable: true).obs;
  ...
  onAssignStokData(String noItemWarna) async {
    ...
    for (var item in result) {
      ...
      if (!alreadyExist) {
        ...
        lsQtyStockIn.add(TextEditingController());
  ...
  Future<String?> validateInsertRequestStokMasuk() async {
    // ===== Validasi header =====
    if (fieldNoStokMasuk.text.trim().isEmpty) {
      return "No permintaan stok masuk wajib diisi";
    }
    ...
    for (int i = 0; i < lsStockFound.length; i++) {
      final qtyText = lsQtyStockIn[i].text.trim();

      if (qtyText.isEmpty) continue;

      final qty = double.tryParse(qtyText);
      if (qty == null || qty <= 0) {
        return "Jumlah masuk harus lebih dari 0";
      }
    ...
    if (!hasValidQty) {
      return "Minimal satu item harus memiliki jumlah masuk";
    }

    return null; // valid
  }

  Future<void> insertRequestStokMasuk() async {
    try {
      isLoading(true);

      final validationMessage = await validateInsertRequestStokMasuk();
      ...
      await db.transaction((txn) async {
        // insert header
        await txn.insert('permintaan_stok_masuk', {
          'no_permintaan_masuk': fieldNoStokMasuk.text,
          'username': username.value,
          'tgl_permintaan_masuk': fieldTanggal.text,
        ...
        for (int i = 0; i < lsStockFound.length; i++) {
          ...
          await txn.insert('detail_permintaan_masuk', {
            'no_permintaan_masuk': fieldNoStokMasuk.text,
  ...
  void onClearData() {
    ...
    // clear list stok & qty
    for (var controllerQty in lsQtyStockIn) {
